...
-- Allow admins to manage all deliveries
...
-- Allow drivers to view assigned deliveries
...
-- Allow drivers to manage their own assignments
CREATE POLICY "Drivers can manage their own assignments" ON driver_deliveries
  FOR ALL USING (driver_id = auth.uid());

-- Allow admins to view all driver assignments
CREATE POLICY "Admins can view all driver assignments" ON driver_deliveries
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM user_profiles 
      WHERE id = auth.uid() AND type = 'admin'
